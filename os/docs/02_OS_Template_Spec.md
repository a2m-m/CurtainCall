...
### **4.1 `os/os-template.yml`（Instance設定の唯一の入口）**
...
### **4.2 `os/scripts/run`（唯一の実行口）**

人もCIも **これだけ叩けばよい**を実現する。

#### **4.2.1 サブコマンド**

* `os/scripts/run format`  
* `os/scripts/run lint`  
* `os/scripts/run typecheck`  
* `os/scripts/run test`  
* `os/scripts/run build`  
* `os/scripts/run ci`（標準：lint → typecheck（あれば）→ test）  
* `os/scripts/run doctor`（設定不足/環境不足を診断）  
* `os/scripts/run print-config`（最終的に解釈された設定を表示）

#### **4.2.2 期待される振る舞い（重要）**

* `os/os-template.yml` がない/壊れている場合：**何が不足かを表示して非0終了**  
...
* MUST `.ai-context.md`  
* MUST `os/os-template.yml`（テンプレに“例＋TODO”で同梱）  
* MUST `scripts/`  
* MUST `.github/`  
* MUST `docs/`  
* SHOULD `CHANGELOG.md`  
...
* SHOULD `AGENTS.md`（Codex 固有ルール）  
* SHOULD `.codex/skills/`（反復作業のモジュール化）  
* SHOULD `.codex/skills/<name>/SKILL.md`（各スキルの定義）
...
3. Contract（`os/os-template.yml` / `os/scripts/run`）  
...
* 生成物は “小さく分けてPR” を推奨  
* 例：IssueのACを満たす／勝手に巨大改修しない／設定は `os/os-template.yml` に寄せる
...
* What / Why（Issueリンク）  
* How to test（`os/scripts/run ...` を書く）  
* Risk / Rollback  
* Screenshots / Logs（任意）  
* “今回やらないこと”（未来のIssueへ）
...
* やることは固定：  
  1. checkout  
  2. `./os/scripts/run doctor`  
  3. `./os/scripts/run ci`

これでCIは言語を知らずに済む。言語差分は `os/os-template.yml` へ。
...
* 言語依存が強い場合は “枠だけ”を用意し、`os/os-template.yml.features.dependency_scan` でON/OFF
...
Iteration Cycle（①〜⑥）で反復する作業を **QA/Architect 系 Skills** としてモジュール化する。

* Claude Code では `.claude/skills/<name>/SKILL.md`、Codex では `.codex/skills/<name>/SKILL.md` に配置する  
* Template に含むスキル（不変）と、Instance で追加するスキル（可変）を区別する  
* エージェント間で対応するスキルは同名・同等責務を維持する  
...
* `os/os-template.yml` がテンプレとして同梱されている（例＋TODOでも良い）  
* `os/scripts/run doctor` が「不足項目」を一覧で出せる  
* `os/scripts/run ci` が（設定が埋まれば） lint/test を実行できる  
* GitHub Actions がPRで起動し、`./os/scripts/run doctor` と `./os/scripts/run ci` を走らせる  
...
* `scripts/init` で `os/os-template.yml` が埋まり、Quickstart通りに `os/scripts/run ci` が実行できる  
