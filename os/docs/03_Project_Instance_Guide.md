...
- リポジトリ（最低：README / os-template.yml / docs / .ai-\* / 言語設定ファイル）  
...
4) **Validate（合否判定）**  
   `./scripts/run doctor` と `./scripts/run ci` を通す。失敗したらログを貼り、原因特定→修正案→再検証の短いループにする。

### 5.3 Outputs（AIが作る成果物：固定）

- `docs/vision.md`（テンプレを埋めたもの）  
- `docs/architecture.md`（テンプレを埋めたもの）  
- `docs/decisions/README.md`（運用ルール）＋ `docs/decisions/0001-*.md`（主要決定のADR）  
- `os-template.yml`（commands.lint/testは必須）  
...
### 6.1 os-template.yml（Instance設定の唯一の入口）
...
### 6.2 scripts/run（唯一の実行口）

人もCIも `scripts/run` だけ叩けばよい、を実現する。  
...
- 生成物は“小さく分けてPR”  
- 設定は os-template.yml に寄せる、巨大改修を勝手にしない
...
CIは「言語を知らない」構造にする。やることは固定で doctor → ci、言語差分は `os-template.yml` に寄せる。
...
- `./scripts/run doctor` が不足を出さない（出たら“何をどこに書くか”を埋める）  
- `./scripts/run ci` が通る（lint/testが回る）  
...
### A2. docs/architecture.md（最小スケッチ）
...
Interview結果の“唯一の事実置き場”。AIはここを更新→docs/os-template.ymlへ反映。
...
\- 03\_Project\_Instance\_Guideに従う。Templateのワークフロー/スクリプト本体の改造は避け、差分は os-template.yml とプロジェクト固有ファイルで吸収する。

\- まずリポジトリと既存ドキュメントを読み、推測できる項目は埋める。

\- 不足情報は最大10問の問診で回収する。答えが不明ならデフォルト案を提示し、ADRに「見直し条件」を書いて仮決めする。

\- 出力は必ずファイルパス単位で提示する：docs/vision.md, docs/architecture.md, docs/decisions/..., os-template.yml, .ai-context.md（可能なら project\_profile.yml も）
...
## 付録D：os-template.yml（言語別サンプル：commandsだけ差し替え）
