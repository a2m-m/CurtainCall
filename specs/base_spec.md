...
5. **ウォッチ**: 相手は **クラップ/ブーイング** を選択（ブーイング必須ロジックあり）。
...
- **初期配分（確定）**: スタンバイで **セットに 13 枚**を表向き未公開で配置し、**残り 40 枚を 20 枚ずつ**プレイヤーに配る。**山札は残らない**（追加ドローなし）。
...
```json
{
  ...
  "turn": 1,
  "history": [],
  "meta": { "composition": { "total": 53, "joker": 1, "set": 13, "perHand": 20 } },
...
- ルール系は `rules.js` に隔離（ブーイング必須、JOKERボーナス、終了判定、スコア計算）。
- DOM 反映は差分更新（`data-*` とイベント委譲）。
- ユニットテスト（任意）: `rules.spec.js`（終了判定/スコア計算/必須ブーイング）。
...
- **ウォッチ**: `needed >= r` でクラップ無効。文言にカウンタが反映。
...
- **仕様整合性（rulebook/各仕様書 vs SRS）**:
  - **用語/数値/分岐の一致**: デッキ構成、初期手札20枚×2、セット13、終了条件（JOKER登場/SET残1）、JOKERボーナスの挙動、セット任意オープン1回/スポットライト、行動権の帰属（ブーイング的中時＝ブーイング側）に齟齬がない。
